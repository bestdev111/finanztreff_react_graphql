...
import { Option as IndexOption, IndexSelectorComponent } from "components/filters/IndexSelectorComponent/IndexSelectorComponent";
import { Option, OptionSelectorComponent } from "components/filters/OptionSelectorComponent/OptionSelectorComponent";
import { useContext, useState } from "react";
import CommoditySearchContext from './CommoditySearchContext';
...
export function CommodityResultFilter({onChange, ...props}: EtfResultFilterProps) {
    ...
    let [state, setState] = useState<EtfResultFilterState>({
        ...
        replication: (ANY_REPLICATION).value,
        replicationId: ( ANY_REPLICATION).id,
    ...
    if (props.replicationId != state.originalReplicationId || props.distributing != state.originalDistributing) {
        let distributionSelected = DISTRIBUTION_OPTIONS.find(current => current.value == props.distributing);
        let replicationSelected = undefined;
        setState({...state,
            distributing: (distributionSelected || DISTRIBUTION_OPTIONS[0]).value,
            distributingId: (distributionSelected || DISTRIBUTION_OPTIONS[0]).id,
            originalDistributing: props.distributing,
