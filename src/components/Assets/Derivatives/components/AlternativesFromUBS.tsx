                            ...
                            onClick={() => 
                                {
                                    ...
                                    if(item.y > 0){
                                        handleRedirectWithProps(push, props.underlying.underlyings, props.assetClass, props.assetType, props.groupId, props.warrantGroupId.issuer, item.options)
                                    }
                            }
                            } 
...
interface AlternativesFromUBSProps {
    className?: string;
    issuerId?: string;
...
export function AlternativesFromUBS(props: AlternativesFromUBSProps) {
    ...
    let { loading: chartLoading, data: chartData } = useQuery<Query>(loader('./getAlternativeDerivativeProducts.graphql'), {
        variables: { underlyingInstrumentGroupId: underlyingInstrumentGroupId, issuerId: props.issuerId },
        ...
        plotOptions: {
            ...
            series: {
                ...
                point: {
                    events: {
                        click(event:any) {
                                if(event.point.options.clickable){
                                handleRedirectWithProps(push, props.underlying?.underlyings, props.assetClass, props.assetType, props.groupId, props.warrantGroupId?.issuer,event.point.options)
                                }else{
        ...
        <>
            <div className={classNames("content-wrapper", props.className)} style={{ maxWidth: "365px" }}>
                <h3 className="content-wrapper-heading font-weight-bold">
                    Alternativen von {props.warrantGroupId?.issuer?.name}
                </h3>
                {
                    chartLoading ?
...
export function handleRedirectWithProps(variable:any, underlying:any, assetClass: any, assetType: any, groupId: any, issuer?: any, clickedItem?:any) {
    if(clickedItem?.name.split(" ").length > 1){
        ...
        variable({
            ...
            search: '?aclass=' + clickedItemClass + '&type=' + clickedItemType + '&underlying=' + underlying[0].group.id + '&issuerId=' + issuer.id + '&issuerName=' + issuer.name,
        })
    }else{
        variable({
            pathname: '/hebelprodukte/suche',
            search: '?aclass=' + clickedItem.name + '&underlying=' + underlying[0].group.id + '&issuerId=' + issuer.id + '&issuerName=' + issuer.name,
        })
    }
    return false;
}

export default AlternativesFromUBS
