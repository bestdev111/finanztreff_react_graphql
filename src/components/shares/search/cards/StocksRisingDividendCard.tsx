...
interface StocksRisingDividendCardProps extends BaseShareSearchProps {
  children?: ReactNode;
}

interface StocksRisingDividendCardState extends GlobalShareSearchProps {
    change: string
    selectedPeriod: string
}

function StocksRisingDividendCardContentView(props: CriteriaState<StocksRisingDividendCardState>) {
    return (
        <>
            ...
            <FilterDropdownItem<OptionItem & { value: Period }>
                onSelect={(option: OptionItem & { value: Period }) => {
                    props.setCriteria({...props.state, period: option.value, trends: [{
                        keyFigure: ShareSearchKeyFigure.DividendYield, type: TrendType.Period, value: Trend.Positive
                        }],
...
function renderQuery(state: StocksRisingDividendCardState, metaData: ShareSearchContextProps | null): SearchShareCriterion {
    return {
        ...
        trends: state.trends.length > 0 ? state.trends : [{keyFigure: ShareSearchKeyFigure.DividendYield, type: TrendType.Period, value: Trend.Positive}],
    };
}
...
export const StocksRisingDividendCard = wrapShareSearchCard(generateShareSearchCard<StocksRisingDividendCardProps & EnabledProps, StocksRisingDividendCardState>(StocksRisingDividendCardContentView),
    renderQuery,
    {
    ...
    CardResults.stocksRisingDividend,
    ShareSortField.DividendYield,
    ["Dividende"]
);

export default StocksRisingDividendCard;
