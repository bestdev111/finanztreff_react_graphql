...
interface SharesUpdatingAnnuallyCardProps extends BaseShareSearchProps {
  children?: ReactNode;
}

interface SharesUpdatingAnnuallyCardState extends GlobalShareSearchProps {
    status: string
    keyfigure: string
    tag: string
}

function SharesUpdatingAnnuallyCardContentView(props: CriteriaState<SharesUpdatingAnnuallyCardState>) {
    ...
    return <>
        ...
        />
        <ShareSearchCardText text={"letzte 5 Jahre"} />
      </>;
    }

function renderQuery(state: SharesUpdatingAnnuallyCardState, metaData: ShareSearchContextProps | null): SearchShareCriterion {
  return {
    marketCapitalization: state.marketCapitalization,
    period: Period.Last_5Years,
    ranges: state.ranges,
...
export const SharesUpdatingAnnuallyCard = wrapShareSearchCard(
  generateShareSearchCard<
    SharesUpdatingAnnuallyCardProps & EnabledProps,
    SharesUpdatingAnnuallyCardState
  >(SharesUpdatingAnnuallyCardContentView),
  renderQuery,
  {
  ...
  false,
    ...
    CardResults.sharesUpdatingAnnually,
    ShareSortField.CashFlowChangePercent,
  ["Cashflow", "Umsatz", "Gewinn"]
);

export default SharesUpdatingAnnuallyCard;
