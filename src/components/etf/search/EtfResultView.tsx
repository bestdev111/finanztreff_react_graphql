...
import {EtfSortField, SearchEtfCriterion, Query} from "../../../generated/graphql";
import InfiniteScroll from "../../common/scroller/InfiniteScroller";
import {EtfResultFilter} from "./EtfResultFilter";
...
export function EtfResultView(props: EtfResultProps) {
            ...
            >
                {
                    data && data.searchEtf && data.searchEtf.edges.length > 0 ?
                        (data?.searchEtf.edges || []).map((current, index) => {
                            ...
                            return <EtfSummaryCard instrument={current.node} key={index} />
                        })
                        :
