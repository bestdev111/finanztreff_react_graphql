...
function DropdownBody(props: PortfoliosSortingDropdownProps) {
    ...
    let closeAction = useCallback(() => {
        trigInfonline(guessInfonlineSection(), "sorting")
        if (context) {
            context.close();
        }
    }, [context]);
        ...
        <Card className="countries-regions-filter px-3 pt-1 border-0">
            ...
            <Card.Body className="px-0 py-1">
                <div className="d-flex flex-wrap button-container mx-n1 border-bottom-2 border-gray-light py-2">
                    {
                        props.sortOptionsList.map(current =>
...
export function PortfoliosSortingDropdown(props: PortfoliosSortingDropdownProps) {
    let [state, setState] = useState<SortOption>({ sortType: props.sort.sortType, direction: props.sort.direction });
    ...
    return (
        <PageHeaderFilterComponent
            ...
            description={
                <>
                    <span className="d-none d-xl-block">
                        {
                            state.direction ?
                                <SvgImage icon={"icon_arrow_short_fullup_white.svg"} width="20" />
                                : <SvgImage icon={"icon_arrow_short_fulldown_white.svg"} width="20" />
            ...
            <DropdownBody sort={{ sortType: state.sortType, direction: state.direction }} sortOptionsList={props.sortOptionsList} handleSort={(sort: SortOption) => {
                setState({ sortType: sort.sortType, direction: sort.direction })
                props.handleSort({ sortType: sort.sortType, direction: sort.direction })
            }} />
        </PageHeaderFilterComponent>
    );
}

interface PortfoliosSortingDropdownProps {
